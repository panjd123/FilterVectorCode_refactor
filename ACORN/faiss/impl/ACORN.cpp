  ...
  reduction(+: tot_reciprocal) reduction(+: n_node)
   ...
   namespace
   {
      ...
      int hybrid_greedy_update_nearest(
          ...
          float &d_nearest)
      {
         //  std::cout << "=== Enter hybrid_greedy_update_nearest ===" << std::endl;
         //  std::cout << "Initial nearest: " << nearest << ", d_nearest: " <<
         //  d_nearest
         ...
         for (;;)
         {
            ...
            //   std::cout << "Current nearest: " << nearest
            //             << ", distance: " << d_nearest << std::endl;

            size_t begin, end;
            hnsw.neighbor_range(nearest, level, &begin, &end);
            //   std::cout << "Neighbor range for node " << nearest << ": [" <<
            //   begin
            //             << ", " << end << ")" << std::endl;

            // for debugging, collect all neighbors looked at in a vector
            std::vector<std::pair<storage_idx_t, std::vector<int>>>
                neighbors_checked;
            ...
            for (size_t i = begin; i < end; i++)
            {
               ...
               // note that this slows down search significantly but can be useful
               // for debugging auto metadata = hnsw.metadata_multi[v]; if
               // (debugSearchFlag) {
               //     neighbors_checked.push_back(std::make_pair(v, metadata));
               //     debug_search(
               //             "------------checking neighbor: %d, metadata: %d\n",
               //             v,
               //             metadata);
               // }
...
#pragma omp critical
      ...
      // backtracking things
      std::vector<storage_idx_t> ep_per_level(
          max_level); // idx of nearest node per level
   ...
   namespace
   {
          ...
          const SearchParametersACORN *params = nullptr)
      {
         ...
         { // candidates is heap of size max(efs, k)
            float d0 = 0;
          ...
          const SearchParametersACORN *params = nullptr)
      {
         ...
         { // candidates is heap of size max(efs, k)
            ...
            for (size_t j = begin; j < end; j++)
            {
               ...
               if (if_bfs_filter) // 原始ACORN：bfs的时候过滤，不符合不再扩展邻
               {                  // 搜索和压入堆的时候都限制了filter
                  if (filter_map[v1])
                  {
               ...
               else // 修改版：bfs时某个点不符合，也会扩展它的邻居
               {
                  // ================== 调试代码 ==================
                  // #pragma omp critical
                  // {
                  //    // 使用 #pragma omp critical 来确保打印操作的原子性，防止多线程输出混乱
                  //    printf("[DEBUG] Thread %d entered hybrid search path (if_bfs_filter=false).\n",
                  //             omp_get_thread_num());
                  //    fflush(stdout);
                  // }
                  // ================================================
       ...
       const SearchParametersACORN *params) const
   {
      ...
      { // common branch
         ...
         //   std::cout << "ndis_upper: " << ndis_upper << std::endl;

         int ef = std::max(efSearch, k);
         if (search_bounded_queue)
         { // this is the most common branch
            // std::cout << "enter search_bounded_queue" << std::endl;
            debug("%s\n", "reached search bounded queue");
